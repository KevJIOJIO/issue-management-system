...
export default function ProjectDashboard({ onViewIssues }) {
  ...
  const [cpuMenuOptions, setCpuMenuOptions] = useState(null); // snapshot while CPU menu open
  const [gpuMenuOptions, setGpuMenuOptions] = useState(null); // snapshot while GPU menu open
  const prevCpuOpen = useRef(false);
  const prevGpuOpen = useRef(false);
  ...
  // Keep CPU/GPU menu options stable while menu is open; refresh only when toggling
  useEffect(() => {
    if (cpuMenuOpen && !prevCpuOpen.current) {
      setCpuMenuOptions(availableCPUs);
    }
    if (!cpuMenuOpen && prevCpuOpen.current) {
      setCpuMenuOptions(null);
    }
    prevCpuOpen.current = cpuMenuOpen;
  }, [cpuMenuOpen, availableCPUs]);

  useEffect(() => {
    if (gpuMenuOpen && !prevGpuOpen.current) {
      setGpuMenuOptions(availableGPUs);
    }
    if (!gpuMenuOpen && prevGpuOpen.current) {
      setGpuMenuOptions(null);
    }
    prevGpuOpen.current = gpuMenuOpen;
  }, [gpuMenuOpen, availableGPUs]);
        ...
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap gap-2 items-center relative" ref={cpuMenuRef}>
            ...
            {cpuMenuOpen && (
              ...
              >
                {Object.entries(cpuMenuOptions ?? availableCPUs).map(([series, models]) => (
          ...
          <div className="flex flex-wrap gap-2 items-center relative" ref={gpuMenuRef}>
            ...
            {gpuMenuOpen && (
              ...
              >
                {Object.entries(gpuMenuOptions ?? availableGPUs).map(([series, models]) => (
                  <div key={series} className="relative group/series">
                    <div className="px-3 py-2 text-sm text-gray-700 hover:bg-gray-100 cursor-pointer flex justify-between items-center">
                      {series}
                      <ChevronDown size={14} className="transform rotate-[-90deg] text-gray-400 group-hover/series:text-gray-600" />
